//catch action, transfer to browser, return action id to runner, catch action done, return result to runner
...
it('should catch action done', function(done) {
    Browser.socket.send('run_action_done', {
        action_id: action_id,
        result: {a: 5, b: 7}
    });
    done();
...
it('should return action result to runner', function(done) {
    Runner.socket.once('run_action_done', function(data) {
        assert(data);
        assert(data.action_id == action_id);
        assert(data.result);
        assert(data.result.a == 5);
        assert(data.result.b == 7);
