//catch action, transfer to browser, return action id to runner, browser unresponsive, send error to runner
return;
...
it('should receive browser unresponsive', function(done) {
    Browser.socket.socket.close();
    done();
});

it('should send an error to runner', function(done) {
    Runner.socket.once('browser_unresponsive', function(data) {
        assert(data);
        assert(data.browser_id);
        done();
    });
...
it('should catch action done', function(done) {
    Browser.socket.send('run_action_fail', {action_id: action_id});
    done();
});
